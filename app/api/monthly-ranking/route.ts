...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: 'Failed to load monthly ranking.' },
        { status: 500 }
      )
    }
    ...
    const topOnePercentValue = topOnePercentRow
      ? Number(topOnePercentRow.econ_value ?? 0)
      : 0
    const amountToTopOnePercent =
      rank && topOnePercentRank >= rank
        ? 0
        : Math.max(0, topOnePercentValue - userEconValue)

    const neighborStart = Math.max(0, (hasRecord ? userIndex : 0) - 2)
    const neighborEnd = Math.min(rows.length, neighborStart + 5)
    const surroundingUsers = rows.slice(neighborStart, neighborEnd).map(
      (row, index) => ({
        userId: row.user_id,
        econValue: Number(row.econ_value ?? 0),
